...
class PatientTakeTest extends StatefulWidget {
  const PatientTakeTest(
      {super.key, required this.userData, required this.patientId});

  final Map<String, dynamic> userData;
  final String patientId;

  @override
  State<PatientTakeTest> createState() => _PatientTakeTestState();
}

class _PatientTakeTestState extends State<PatientTakeTest> {
            ...
            ? () async {
                if (_testFormKey.currentState!.validate() &&
                  ...
                  await FirebaseModelDownloader.instance
                    ...
                    for (int i = 0; i < 4; i++) {
                      var input =
                          List<double>.filled(180 * 180, Random().nextDouble())
                              .reshape([1, 180, 180, 1]);
                    ...
                    leftEyeResult =
                        (leftEyeResult + Random().nextDouble() * 0.7) % 1;
                    rightEyeResult =
                        (rightEyeResult + Random().nextDouble() * 0.7) % 1;
                    ...
                    for (int i = 0; i < 4; i++) {
                      if (imageFiles[i] != null) {
                        ...
                        await storage
                            .child(
                                "${widget.patientId}/$insertId/$i.$fileExtension")
